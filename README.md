...
- **I still have not been able to compile or run this anywhere** — no
  .NET SDK, Docker daemon, or Postgres instance is available in the
  environment that produced it. You've said you'll build, run, and test
  it yourself from here, which is the right call — I can write code
  carefully and review it for obvious mistakes (and did: fixed a wrong
  Playwright type name, `BrowserLaunchOptions` → `BrowserTypeLaunchOptions`,
  and cross-checked every Playwright selector against the actual
  rendered form field names/button text during this pass), but I cannot
  substitute for an actual `dotnet build` run. Treat first build as an
  expected step where small issues may still surface.
...
`playwright install --with-deps chromium` on the CI runner.
