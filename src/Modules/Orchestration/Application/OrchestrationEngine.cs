...
public sealed class OrchestrationEngine
{
        ...
        CancellationToken ct)
    {
        ...
        await _events.PublishAsync(NeuralEventTypes.TaskStarted, new { task.Id, task.Title }, ct);
        task.Start();
        primaryAgent.SetStatus(AgentStatus.Executing);
        await _events.PublishAsync(NeuralEventTypes.AgentExecuting, new { primaryAgent.Id, primaryAgent.Name }, ct);
        ...
        while (step < maxSteps)
        {
            ...
            {
                await _events.PublishAsync(NeuralEventTypes.ContextExhausted, new { currentAgent.Id, response.TokenUsage.UsageRatio }, ct);
            ...
            if (response.TokenUsage.UsageRatio >= 0.96 && level != ContextLevel.Exhausted)
            {
                ...
                await _events.PublishAsync(NeuralEventTypes.TaskCompleted, new { task.Id }, ct);
                if (_notifications is not null)
