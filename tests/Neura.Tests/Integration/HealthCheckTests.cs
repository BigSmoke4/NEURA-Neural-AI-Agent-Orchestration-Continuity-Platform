...
public class HealthCheckTests : IClassFixture<NeuraWebApplicationFactory>
{
    ...
    public async Task Register_ThenLogin_Succeeds()
    {
        ...
        Assert.Contains("/Brain", registerResponse.Headers.Location?.ToString() ?? string.Empty);
        ...
        Assert.Contains("/Brain", loginResponse.Headers.Location?.ToString() ?? string.Empty);
